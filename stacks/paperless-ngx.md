...
# 🚀 Paperless-NGX auf Proxmox LXC mit NFS-Storage & Docker
...
- **NAS-Storage `/volume1/docker` als NFS-Mount** für Paperless-Daten
...
## 🛠 4. Docker-Compose für Paperless-NGX
### 1️⃣ Erstelle eine `.env`-Datei für Konfigurationswerte
```bash
...
PAPERLESS_ADMIN_USER=deleven
PAPERLESS_ADMIN_PASSWORD=supergeheimespasswort
PAPERLESS_SECRET_KEY=ein-super-geheimer-schluessel
...
### 2️⃣ Erstelle das `docker-compose.yml`
```bash
...
services:
  ...
  webserver:
    ...
    ports:
      - 8810:8000
    volumes:
      - /usr/src/paperless/data:/usr/src/paperless/data
...
### 3️⃣ Starte Paperless
```bash
docker compose up -d
docker logs -f paperless-ngx-webserver
```

---

## 🚀 Fazit
✅ NAS-Storage korrekt eingebunden
✅ LXC-Container mit NFS-Mount konfiguriert
✅ Paperless mit Docker-Compose aufgesetzt
